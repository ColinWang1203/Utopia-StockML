...
def parse_command_line():
    parser = ArgumentParser(description='Can you find your apples?')
    ...
    parser.add_argument('-s', dest='simple_mode', action='store_true',  help='-s for just adding today\'s data and remove one oldest')
...
def download_apples(day_start,day_end,Apple_location):
...
def main():
    ...
    if args.simple_mode:
        ...
        if os.stat(csv_file_name).st_size == 0:
            print(today+'.csv is not valid.')
            os.remove(csv_file_name)
            return

        print(today+'.csv is downloaded.')

        #delete the oldest file by finding the first sorted file
        file_list = sorted(os.listdir(Apple_location))
        os.remove(Apple_location+file_list[0])
        print(file_list[0]+' is removed')
        return
    
    # skip the data that is already downloaded
    day_start_with_skip = max([x.split('.')[0] for x in os.listdir('./Apples')])
    download_apples(day_start_with_skip,day_end,Apple_location)
